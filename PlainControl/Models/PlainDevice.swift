...
/// SwiftData model for a PlainApp device.
@Model
final class PlainDevice {
    ...
    init(
        ...
        self.id = id
        self.name = name
        self.ipsJSON = (try? JSONEncoder().encode(ips)).flatMap { String(data: $0, encoding: .utf8) } ?? "[]"
    ...
    func setIPs(_ ips: [String]) {
        ipsJSON = (try? JSONEncoder().encode(ips)).flatMap { String(data: $0, encoding: .utf8) } ?? "[]"
    }

    var primaryIP: String? { getIPs().first }
    ...
    var initURL: URL? {
        guard let ip = primaryIP else { return nil }
        if httpsPort > 0 { return URL(string: "https://\(ip):\(httpsPort)/init") }
    ...
    var deviceTypeDisplay: String { deviceType.capitalized }
    var deviceIconName: String { "iphone" }
